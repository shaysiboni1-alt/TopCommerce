...
function enrichMeta(meta) {
  ...
  if (base.callSid && !base.call_sid) base.call_sid = base.callSid;
  if (base.streamSid && !base.stream_sid) base.stream_sid = base.streamSid;
  return base;
}

function emit(level, msg, meta) {
  const normalized = normalizeArgs(msg, meta);
  const debugEnabled = isTruthyEnv(env.MB_DEBUG);
  ...
  const finalMeta = enrichMeta(normalized.meta);
  if (finalMeta && Object.keys(finalMeta).length) {
    line.meta = safeJson(finalMeta);
  }
