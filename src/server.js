...
const { runtimeSettingsRouter } = require("./routes/runtimeSettings");
...
const { proxyRecordingMp3, publicRecordingUrl } = require("./utils/twilioRecordings");
...
app.use(runtimeSettingsRouter);
app.use(debugCallsRouter);
...
app.post("/twilio-recording-callback", (req, res) => {
  try {
    ...
    const publicUrl = recordingSid ? publicRecordingUrl(recordingSid) : (recordingUrl || null);

    if (callSid) {
      setRecordingForCall(callSid, {
        recordingSid: recordingSid || null,
        recordingUrl: publicUrl || null,
      });
    }
...
const server = app.listen(env.PORT, "0.0.0.0", async () => {
  ...
  try {
    ...
    try {
      ...
      logger.warn("Opening/system warmup failed", {
